...
from Models.StateProp import GRU_Linear
...
class TrajWeaverUNet(nn.Module):
    ...
    def getStateShapes(self, traj_len):
        shapes = [(self.c_list[i], 1) for i in range(len(self.c_list))]
...
class Linkage(nn.Module):
    def __init__(self, in_shapes: List[Tuple[int, int]], max_t: int):
        ...
        for shape in in_shapes:
            c = shape[0]
            self.gru_cells.append(GRU_Linear(c, c, max_t))
